...
public class HeadHidingPacketListener implements PacketListener {
    ...
    public HeadHidingPacketListener(ServiceRegistry registry) {
        this.registry = registry;
    }

    @Override
    public void onPacketSend(@NotNull PacketSendEvent event) {
        if (!registry.getConfigService().isHideFoundHeads()) {
            return;
        }
    ...
    public void removeFoundHead(Player player, UUID headUuid) {
        ...
        if (headLocation != null) {
            ...
            if (player.getWorld().equals(loc.getWorld())) {
                HeadBlocks.getScheduler().runTaskLater(loc, () -> {
                    BlockData data = loc.getBlock().getBlockData();
                    HeadBlocks.getScheduler().runTask(player, () -> player.sendBlockChange(loc, data));
            ...
            for (var headUuid : previouslyHiddenHeads) {
                ...
                if (headLocation != null && player.getWorld().equals(headLocation.getLocation().getWorld())) {
                    ...
                    HeadBlocks.getScheduler().runTaskLater(location, () -> {
                        HeadBlocks.getScheduler().runTask(player, () -> player.sendBlockChange(location, location.getBlock().getBlockData()));
