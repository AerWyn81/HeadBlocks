...
public class GuiService {
    ...
    public static void openHintGui(Player p) {
        ...
        if (!guiViewHint.containsKey(p.getUniqueId())) {
            guiViewHint.put(p.getUniqueId(), HintMode.SOUND);
        }

        var playerSelectedMode = guiViewHint.get(p.getUniqueId());
                ...
                var orderItemGui = new ItemGUI(new ItemBuilder(getHeadItemStackFromCache(headLocation))
                        ...
                        .addOnClickEvent(event -> {
                            if (event.getClick() == ClickType.MIDDLE) {
                                guiViewHint.put(p.getUniqueId(), playerSelectedMode.next());
                            } else {
